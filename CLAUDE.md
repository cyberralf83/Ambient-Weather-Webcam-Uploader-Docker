...
**Status web page.** Optional, on by default. `entrypoint.sh` starts `httpd -h /var/www` on `STATUS_PORT` (default 8080) before launching `crond`, after symlinking `/home/root/image.jpg` into `/var/www/image.jpg` so the live snapshot is served. The httpd binary comes from Alpine's `busybox-extras` package as a standalone `/usr/sbin/httpd` — **not** as a sub-applet of the main `busybox` binary, which is why we call it directly rather than via `busybox httpd`. `ams-cam-upload.sh` maintains counters in `/home/root/stats` (key=value, sourced via `.`) and re-renders `/var/www/index.html` at the end of every run via `update_stats` and `render_status_page`. Both calls are guarded with `|| true` so a status-rendering failure never breaks the upload pipeline. Disable entirely with `STATUS_ENABLED=false`.
...
- **`INTERVAL_MINUTES` vs `CRON_SCHEDULE`**: `INTERVAL_MINUTES` (1-60) is the user-facing simple knob — entrypoint translates it into `CRON_SCHEDULE="*/N * * * *"` before validation. If both are set, `INTERVAL_MINUTES` wins. Don't add a similar wrapper for hour-level schedules; users wanting that fall back to raw `CRON_SCHEDULE`.
